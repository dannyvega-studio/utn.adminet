<?php
	...
	$sentencia_select=$con->prepare('SELECT * FROM departamento ORDER BY ID_Departamento ASC');
	...
	if(isset($_POST['btn_buscar'])){
		...
		$select_buscar=$con->prepare('
			SELECT * FROM departamento WHERE ID_Departamento LIKE :campo OR ID_Director LIKE :campo;'
		);
...
<body>
  ...
  <!-- Fin Navbar -->
	...
	<div class="contenedor">
		<h2>Registro de los departamentos</h2>
		<div class="barra__buscador">
			<form action="" class="formulario" method="post">
				<input type="text" name="buscar" placeholder="Buscar Empresa" 
				value="<?php if(isset($buscar_text)) echo $buscar_text; ?>" class="input__text">
				<input type="submit" class="btn" name="btn_buscar" value="Buscar">
				<a href="vistas/registro_departamentos.php" class="btn btn__nuevo">Nuevo</a>
			</form>
		</div>
		<table id="tablaempsoliciante" class="table table-striped table-bordered table-condensed"  style="width: 100%">
			<tr class="head">
				<th>Nombre de departamento</th>
                            <th>Identificador departamento</th>
                            <th>Identificador director</th>
			...
			<?php foreach($resultado as $fila):?>
				<tr >
					...
					<td><?php echo $fila['ID_Director']; ?></td>
					<td><a href="vistas/actualizar_departamentos.php?ID_Departamento=<?php echo $fila['ID_Departamento']; ?>"  class="btn__update" >Editar</a></td>
					<td><a href="crud/departamentodelete.php?ID_Departamento=<?php echo $fila['ID_Departamento']; ?>" class="btn__delete">Eliminar</a></td>
				</tr>
			<?php endforeach ?>
